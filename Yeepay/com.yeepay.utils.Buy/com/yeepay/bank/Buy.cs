namespace com.yeepay.bank
{
    ...
    public abstract class Buy
    {
        ...
        public static string CreateBuyUrl(string p2_Order, string p3_Amt, string p4_Cur, string p5_Pid, string p6_Pcat, string p7_Pdesc, string p8_Url, string p9_SAF, string pa_MP, string pd_FrpId, string pr_NeedRespone, string partnerGatawayID, string partnerKey, string authorizationURL)
        {
            string aValue = "";
            aValue = ((((((aValue + "Buy") + partnerGatawayID + p2_Order) + p3_Amt + p4_Cur) + p5_Pid + p6_Pcat) + p7_Pdesc + p8_Url) + p9_SAF + pa_MP) + pd_FrpId + pr_NeedRespone;
            string hmac = Digest.HmacSign(aValue, partnerKey);
            logstr(p2_Order, aValue, hmac);
            string str3 = "";
            return ((((((((((((((str3 + authorizationURL + "?p0_Cmd=Buy") + "&p1_MerId=" + partnerGatawayID) + "&p2_Order=" + HttpUtility.UrlEncode(p2_Order, Encoding.GetEncoding("gb2312"))) + "&p3_Amt=" + p3_Amt) + "&p4_Cur=" + p4_Cur) + "&p5_Pid=" + HttpUtility.UrlEncode(p5_Pid, Encoding.GetEncoding("gb2312"))) + "&p6_Pcat=" + HttpUtility.UrlEncode(p6_Pcat, Encoding.GetEncoding("gb2312"))) + "&p7_Pdesc=" + HttpUtility.UrlEncode(p7_Pdesc, Encoding.GetEncoding("gb2312"))) + "&p8_Url=" + HttpUtility.UrlEncode(p8_Url, Encoding.GetEncoding("gb2312"))) + "&p9_SAF=" + p9_SAF) + "&pa_MP=" + HttpUtility.UrlEncode(pa_MP, Encoding.GetEncoding("gb2312"))) + "&pd_FrpId=" + pd_FrpId) + "&pr_NeedResponse=" + pr_NeedRespone) + "&hmac=" + hmac);
        }
        ...
        public static BuyRefundOrdResult RefundOrd(string pb_TrxId, string p3_Amt, string p4_Cur, string p5_Desc,string partnerGateWayID,string partnerKey,string queryGatewayOrderUrl)
        {
            string aValue = "";
            aValue = ((aValue + "RefundOrd" + partnerGateWayID) + pb_TrxId + p3_Amt) + p4_Cur + p5_Desc;
            string hmac = Digest.HmacSign(aValue, partnerKey);
            logstr(pb_TrxId, aValue, hmac);
            string para = "";
            para = ((((((para + "?p0_Cmd=RefundOrd") + "&p1_MerId=" + partnerGateWayID) + "&pb_TrxId=" + pb_TrxId) + "&p3_Amt=" + p3_Amt) + "&p4_Cur=" + p4_Cur) + "&p5_Desc=" + HttpUtility.UrlEncode(p5_Desc, Encoding.GetEncoding("gb2312"))) + "&hmac=" + hmac;
            string strUrl = HttpUtils.SendRequest(queryGatewayOrderUrl, para);
            string str5 = FormatQueryString.GetQueryString("r0_Cmd", strUrl, '\n');
            string str6 = FormatQueryString.GetQueryString("r1_Code", strUrl, '\n');
            string str7 = FormatQueryString.GetQueryString("r2_TrxId", strUrl, '\n');
