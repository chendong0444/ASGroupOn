...
namespace AS.Common.Utils.api
{
    ...
    public abstract class RequestBase
    {
        ...
        /// <returns></returns>
        protected T GetResponseData<T>(string requestUrl, Parameters parameters, string openid)
            where T : ResponseObject
        {
            return this.GetResponseData<T>("GET", requestUrl, parameters, null, openid);
        }
        /// <summary>
        /// 采用POST请求并获取输出的数据
        /// </summary>
        ...
        /// <returns></returns>
        protected T GetResponseData<T>(string requestUrl, Parameters parameters, Files files, string openid)
            where T : ResponseObject
        {
            return this.GetResponseData<T>("POST", requestUrl, parameters, files, openid);
        }
        /// <summary>
        /// 请求并获取输出的数据
        /// </summary>
        ...
        /// <returns></returns>
        protected virtual T GetResponseData<T>(string requestMethod, string requestUrl, Parameters parameters, Files files, string openid)
            where T : ResponseObject
        {
            this.LastError = null;
            this.AddOAuthParameter(requestMethod, requestUrl, parameters, openid);
        ...
        /// <returns></returns>
        protected virtual void AddOAuthParameter(string requestMethod, string requestUrl, Parameters parameters, string openid)
        {
