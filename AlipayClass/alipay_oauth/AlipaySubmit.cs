...
namespace AlipayClass.alipay_oauth
{
    ...
    public class Submit
    {
        ...
        private static Dictionary<string, string> BuildRequestPara(SortedDictionary<string, string> sParaTemp)
        {
            ...
            //过滤签名参数数组
            sPara = Function.FilterPara(sParaTemp);
            //获得签名结果
        ...
        private static string BuildRequestParaToString(SortedDictionary<string, string> sParaTemp)
        {
            ...
            string strRequestData = Function.CreateLinkString(sPara);
        ...
        /// <param name="strButtonValue">确认按钮显示文字</param>
        /// <returns>提交表单HTML文本</returns>
        public static string BuildFormURL(SortedDictionary<string, string> sParaTemp, string gateway, string strMethod, string strButtonValue)
        {

            string strHtml = gateway;
            string urlContent = "";
            //待请求参数数组
            Dictionary<string, string> dicPara = new Dictionary<string, string>();
            dicPara = BuildRequestPara(sParaTemp);
            ...
            foreach (KeyValuePair<string, string> temp in dicPara)
            {
                if (i == 0)
                {
                    urlContent += temp.Key + "=" + HttpContext.Current.Server.UrlEncode(temp.Value);
                }
                else
                {
                    urlContent += "&" + temp.Key + "=" + HttpContext.Current.Server.UrlEncode(temp.Value);
