...
namespace AlipayClass
{
    ...
    public class AlipayFunction
    {
        ...
        /// <param name="sArray">需要拼接的数组</param>
        /// <returns>拼接完成以后的字符串</returns>
        public static string Create_linkstring_urlencode(Dictionary<string,string> sArray)
        {
            int nCount = sArray.Count;
            int i = 0;
            StringBuilder prestr = new StringBuilder();

            foreach (string name in sArray.Keys)
            {
                string itemName = name;
                string itemValue = sArray[name];
                prestr.Append("&");
                if (itemName != "service" && itemName != "")
                    prestr.Append(name + "=" + HttpUtility.UrlEncode(itemValue,Encoding.UTF8));
                else
                    prestr.Append(name + "=" + itemValue);
            }
            if (prestr.ToString().Length > 0)
            {
