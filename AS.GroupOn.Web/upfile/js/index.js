...
validator.check = function (obj, submit) {
    ...
    jQuery.each(datatypes, function (index, type) {
        ...
        //ajax validate 
        if (type == 'ajax' && typeof (submit) == "undefined") {
    ...
    jsubmit.click(function () {
        ...
        if (group == null) {
            ...
            for (i = 0; i < inputs.length; i++) {
                var result = validator.check(inputs.eq(i), true);
                if (!result) {
                    inputs.eq(i).focus();
                    return false;
                }
                ...
                if (radioname.length > 0) {
                    ...
                    for (var j = 0; j < namearr.length; j++) {
                        ...
                        if (radios.length == 0) {
                            ...
                            radios.eq(0).focus();
                            radios.blur(function () { radios.removeClass("errorinput"); });
                            ok = false;
                        }
                        else {
            ...
            for (i = 0; i < inputs.length; i++) {
                ...
                if (!result) {
                    ...
                    inputs.eq(i).removeClass("errorinput");
            }
...
X.post = function(u,data) { return X.ajax(u,data, 'POST'); };
...
X.boxShow = function (innerHTML, mask) {
    ...
    if (typeof (dialog.get(0).scrollWidth) !=null) {
...
window.x_init_hook_validator = function () {
    ...
    jQuery("input[post][action]").click(function () {
        X.post($(this).attr("action"), $("*[post='" + $(this).attr("post") + "']").serializeArray());
    });
...
window.x_init_hook_dealimage = function () {
    ...
    if (teamside) {
        ...
        jQuery('.side #side-business img').each(function () {
            X.misc.scaleimage(this, 236);
        });
    }
    jQuery('#deal-stuff1 img').each(function () { X.misc.scaleimage(this, m); });
    jQuery('div.sbox-content img').each(function () { X.misc.scaleimage(this, 236); });
...
window.x_init_hook_clock = function () {
    if (jQuery("div.deal-timeleft").length == 1 && jQuery("div[num]").length == 0) {
        ...
        jQuery("ul#counter").everyTime(1000, 'counter', e);
    }
    else {
        jQuery("div.deal-timeleft").each(function () {
            ...
            jQuery('#counter' + numid).everyTime(1000, 'counter' + numid, e);
        });
...
X.coupon.bizdialogquery = function () {
    ...
    if (id) return !X.get('/ajax/coupon.aspx?action=bizquery&id=' + encodeURIComponent(id) + "&a=" + Math.random());
    else
        return false;
};
...
X.coupon.invent = function (id, result, p) {
        if (id) {
            return jQuery.ajax({
                type: "POST",
                url: webroot + 'ajax/coupon.aspx?action=reinvent&resultid=' + id + "&p=" + p + "&a=" + Math.random(),
                data: { "result": result },
                dataType: "json",
                success: X.json
            });
...
X.coupon.pinvent = function (id, result, p) {
    ...
    if (id) {
        return jQuery.ajax({
            type: "POST",
            url: webroot + 'ajax/coupon.aspx?action=preinvent&resultid=' + id + "&p=" + p + "&a=" + Math.random(),
            data: { "result": result },
            dataType: "json",
            success: X.json
        });
...
X.coupon.checkintory = function (id, result, obj) {

    if (id) return !X.get(webroot + 'ajax/ajax_intory.aspx?proid=' + encodeURIComponent(id) + "&result=" + result + "&obj=" + obj + "&a=" + Math.random());
    else
        return false;
};
    ...
    if (id && secret)
     {
        ...
        var ask = jQuery('#coupon-dialog-bizconsume').attr('asks');
        return confirm(ask) && !X.get('/ajax/coupon.aspx?action=bizconsume&id=' + encodeURIComponent(id) + '&secret=' + encodeURIComponent(secret) + '&selectpart=' + encodeURIComponent(selectpart) +'&shangjiasecret=' + encodeURIComponent(shangjiasecret) +"&a=" + Math.random());
