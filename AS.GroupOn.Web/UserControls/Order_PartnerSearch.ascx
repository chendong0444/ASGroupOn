   ...
   protected string keys = FileUtils.GetKey();
        ...
        get
        {
            if (express == String.Empty)
            {
               using(IDataSession session=AS.GroupOn.App.Store.OpenSession(false))
                {
                    table = session.Orders.SelOrder(orfilter);
                }
                express = express + "<option value='0'></option>";
                foreach(Hashtable ht in table)
                {
                    express = express + "<option value='" + ht["id"] + "'>" + ht["name"] + "(" + ht["num"] + ")</option>";
                }
            }
            return express;
        }
    ...
    public string GetWhereString()
    {
        ...
        if (SearchExpress)
        {
            ...
            IList<Hashtable> tb = null;
            OrderFilter filter = new OrderFilter();
            filter.Partner_id = Helper.GetInt(CookieUtils.GetCookieValue("partner",keys), 0);
            using(IDataSession session=AS.GroupOn.App.Store.OpenSession(false))
            {
                 tb = session.Orders.Sel(filter);
            }
            express = express + "<option value='0'></option>";
             foreach(Hashtable ht in tb)
            {
                if (Helper.GetInt(ht["Id"],0) == request_express)
                    express = express + "<option selected value='" + ht["Id"] + "'>" + ht["Name"] + "(" + ht["num"] + ")</option>";
                else
                    express = express + "<option value='" + ht["Id"] + "'>" + ht["Name"] + "(" + ht["num"] + ")</option>";
            }
...
<li>日期:
    <input name="firstTime" id="firstTime" datatype="date" value="<%=request_firstTime %>"
        class="h-input" style="width: 70px;" type="text" onclick="WdatePicker();">
    &nbsp;至
    <input name="endTime" id="endTime" datatype="date" value="<%=request_endTime %>"
        class="h-input" style="width: 70px;" type="text" onclick="WdatePicker();">
    <select name="time" id="time" class="h-input" style="width:50px;">
        <option value=""></option>
