...
<div class="sbox side-business">
    ...
    <div class="sbox-content">
        ...
        <div class="tip">
            ...
            <% if (display)
               { %>
            <a style="display: block; color: Black;" href="<%=getTeamPageUrl(team.Id)%>"><b><%=i%>、<%=team.Title%></b></a>
            <% }
               else
               {%>
            <a style="display: none; color: Black;" href="<%=getTeamPageUrl(team.Id)%>"><b><%=i%>、<%=team.Title%></b></a>
            <%}%>
            <%if (team.Image != String.Empty)
              { %><p><a href="<%=getTeamPageUrl(team.Id)%>">
                <img src="<%=ImageHelper.getSmallImgUrl(team.Image) %>" width="208" border="0" /></a></p>
            <%} %>
            <div class="yigoumai">
                <p class="xyigm"><%=team.Now_number %> <span style="color: #666">人已购买</span></p>
                <p><a href="<%=getTeamPageUrl(team.Id)%>">
