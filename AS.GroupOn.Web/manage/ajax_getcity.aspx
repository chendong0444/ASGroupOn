...
<script runat="server">
    ...
    protected override void OnLoad(EventArgs e)
    {
        ...
        int error = Helper.GetInt(Request["as"],0);
        ...
        if (id != String.Empty && id != "" && !IsPostBack)
        {
            GetcityList(id);
        }
        if (cityid != "" && cityid!= string.Empty && citypid != "" && citypid != string.Empty)//初始化二级城市
        {
            initData(cityid, citypid);
        }
        else if (strLetter != String.Empty && strLetter != "")//初始化弹出层
        {
            initData(strLetter);
        }
        else
    ...
    {
        string html = "<select  class='f-input' pid='" + id + "' datatype='require' require='true' msg='请选择省市区' msgid='errorcitylist' group='a' onchange='changecity(" + id + ")'";
    ...
    private void GetcityList(string id)
    {
        ...
        string html = "<select class='f-input' id='selecitys' pid='" + pid + "' datatype='require' require='true' msg='请选择省市区' msgid='errorcitylist' group='a' onchange='changecity(" + pid + ")'";
        if (pid == 0)
