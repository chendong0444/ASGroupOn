...
<script runat="server">
    ...
    protected override void OnLoad(EventArgs e)
    {
        base.OnLoad(e);
        //判断管理员是否有此操作
        if (AdminPage.IsAdmin && AdminPage.CheckUserOptionAuth(PageValue.CurrentAdmin, ActionEnum.Option_Order_Express_Print_NoPrintList))
        {
            SetError("你不具有查看未打印快递的权限！");
        ...
        wherestring = wherestring + " and service!='cashondelivery' and state='pay' and Express='Y' and Express_id>0 and isnull(express_xx,'')<>'已打印' ";
        ...
        {
            list_category = seion.Category.GetByExpress(CF);
        }

        StringBuilder sb2 = new StringBuilder();

        url = url + "&page={0}";
        url = "Dingdan_Weidayin.aspx?" + url.Substring(1);
        ...
        //filter.where1 = " and Express_id>0 ";
                        ...
                        <div class="box clear">
                            <div class="box-content">
                                <div class="head">
                                    ...
                                    <div class="search">
                                        生成订单时间：<input type="text" class="h-input" datatype="date" style="margin-right: 0px;"
                                            name="begintime" <%if(!string.IsNullOrEmpty(Request.QueryString["begintime"])){ %>value="<%=Request.QueryString["begintime"] %>"
                                            <%} %> />--<input type="text" class="h-input" datatype="date" name="endtime" <%if(!string.IsNullOrEmpty(Request.QueryString["endtime"])){ %>value="<%=Request.QueryString["endtime"] %>"
                                                <%} %> />&nbsp;&nbsp; 完成订单时间：<input type="text" class="h-input" style="margin-right: 0px;"
                                                    datatype="date" name="fromfinishtime" style="margin-right: 0px;" <%if(!string.IsNullOrEmpty(Request.QueryString["fromfinishtime"])){ %>value="<%=Request.QueryString["fromfinishtime"] %>"
                                        ...
                                        <select name="express" id="express">
                                            ...
                                            %>
                                            <option value='<%=Category.Id%>' <%if(expressid ==Category.Id){ %> selected="selected"
                                                <%} %>>
                                                <%=Category.Name%>(<%=Category.Num%>)</option>
                                            <%
                                                }
                                                                  }
                                            %>
...
category = order.Category;
    ...
    i++;
                                                    ...
                                                    <% 
                                                        if (order.Team_id == 0)
                                                        {
                                                            ...
                                                            if (teamlist != null && teamlist.Count > 0)
                                                            {
                                                                ...
                                                                {%>
                                                    项目ID:<% =teams.Teamid%>
                                                    (<a class="deal-title" href="<%=getTeamPageUrl(teams.Teamid)%>" target="_blank">
                                                        <% =StringUtils.SubString(teams.Team.Title, 70) + "..."%>
                                                    </a>)
                                                ...
                                                <%} %>
                                                <td width='100'>
                                                    <a class="ajaxlink" href='ajax_print.aspx?action=print&amp;id=<%=order.Id %>'>打印快递</a>｜
                                                    <a href='ajax_manage.aspx?action=orderview&amp;orderview=<%=order.Id%>' class="ajaxlink">
                                                        详情</a>
                                                </td>
                                            </tr>
                                            <% 
...
<script>
    function print() {
        X.get(webroot + "manage/ajax_print.aspx?action=print&where=<%=where %>");
