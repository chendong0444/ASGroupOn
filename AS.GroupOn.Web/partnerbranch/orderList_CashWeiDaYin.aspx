...
<script runat="server">
    ...
    protected IList<IOrder> ilistordr = null;
    ...
    private void InitPage()
    {
        ...
        sqll = sql + " and service='cashondelivery' and (state='nocod' or state='pay') and Express='Y' and Express_id>0 and isnull(express_xx,'')<>'已打印'";
        sql = sql + " and service='cashondelivery' and (state='nocod' or state='pay') and Express='Y' and Express_id>0 and isnull(express_xx,'')<>'已打印'";
        ...
        ilistordr = pager.Objects;

        if (ilistordr.Count > 0)
        {
            int i = 0;
            foreach (IOrder orderinfo in ilistordr)
            {
                if ((i + 1) % 2 == 0)
                {
                    stContent.Append("<tr>");
